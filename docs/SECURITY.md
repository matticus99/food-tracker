...
**Implementation:** `csrf-csrf` library (double-submit cookie pattern) in `server/src/index.ts`
...
| Secure flag | Configurable via `COOKIE_SECURE` env var |
| Client integration | `useApi` hook auto-manages token refresh |
...
| CSRF | Double-submit cookie (csrf-csrf) | Done |
