...
cp .env.example .env           # Set APP_PORT to an available port
...
|------------|--------------------------------------|---------|
| `APP_PORT` | Port the application runs on         | `3699`  |
...
| `SESSION_SECRET`    | Secret for CSRF token signing (32+ chars in production) | `my-super-secret-key-change-me` |
| `COOKIE_SECURE`     | Set CSRF cookie secure flag (true for HTTPS) | `true`                          |
